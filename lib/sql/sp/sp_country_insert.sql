DROP FUNCTION sp_country_insert;
CREATE OR REPLACE FUNCTION sp_country_insert
(
	out_num              OUT INTEGER,
	out_str              OUT VARCHAR,
...
BEGIN
	...
	IF v_cnt > 0 THEN
		out_num := 1;
		out_str := 'Country Code already exists';
	...
	IF out_num = 0 THEN
		out_str := 'New Country has been added';
	END IF;
END;
$$ LANGUAGE plpgsql;
